...
func (validator *Validator) Run() error {
	...
	case err := <-sError:
		if err == nil {
			...
			log.Info("validation failed:" + err.Error())
		}
		return err
	}
}
...
func restoreOriginalAddress(l net.Listener, config *Config, c chan<- ReturnCode) {
	...
	for {
		...
		if err != nil {
			log.Info("Error getting original dst: " + err.Error())
			conn.Close()
			continue
		}
...
func (s *Service) Run() error {
	...
	for _, addr := range s.Config.ServerListenAddress {
		...
		if err != nil {
			log.Info("Error on listening:", err.Error())
			continue
		}
...
func (c *Client) Run() error {
	...
	if err != nil {
		log.Infof("Error connecting to %s: %s\n", serverOriginalAddress, err.Error())
		return err
	}
	conn.Close()
	return nil
