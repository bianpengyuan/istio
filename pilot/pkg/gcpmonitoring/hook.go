...
var (
	typeHookTag, _     = tag.NewKey("type")
	eventHookTag, _    = tag.NewKey("event")
	nameHookTag, _     = tag.NewKey("name")
	resourceHookTag, _ = tag.NewKey("resource")
	reasonHookTag, _   = tag.NewKey("reason")
...
func onPilotK8sCfgEvents(f *stats.Float64Measure, tags []tag.Mutator, value float64) {
...
func onGalleyValidationPass(f *stats.Float64Measure, tags []tag.Mutator, value float64) {
	tm := getOriginalTagMap(tags)
	if tm == nil {
		return
	}
...
func onGalleyValidationFailed(f *stats.Float64Measure, tags []tag.Mutator, value float64) {
	tm := getOriginalTagMap(tags)
	if tm == nil {
		return
	}
...
func onPilotXDSPushes(f *stats.Float64Measure, tags []tag.Mutator, value float64) {
	tm := getOriginalTagMap(tags)
	if tm == nil {
		return
	}
	...
	xdsType := strings.ToUpper(string(t[0:3]))
	status := "true"
	if len(t) > 3 {
		status = "false"
	}
...
func onPilotXDSReject(f *stats.Float64Measure, tags []tag.Mutator, value float64) {
...
func onPilotConfigConvergence(f *stats.Float64Measure, tags []tag.Mutator, value float64) {
	stats.Record(context.Background(), configConvergenceMeasuare.M(value))
}

func onPilotXDS(f *stats.Float64Measure, tags []tag.Mutator, value float64) {
	stats.Record(context.Background(), proxyClientsMeasure.M(int64(value)))
}

func onSidecarInjection(f *stats.Float64Measure, tags []tag.Mutator, value float64) {
	status := ""
	switch f.Name() {
