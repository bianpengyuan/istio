...
// NewRedisquotaNoSessionServer creates a new no session server from given args.
func NewRedisquotaNoSessionServer(addr uint16, poolSize int) (*NoSession, error) {
	saddr := fmt.Sprintf(":%d", addr)

	gp := pool.NewGoroutinePool(poolSize, false)
	inf := redisquota.GetInfo()
	s := &NoSession{
		builder: inf.NewBuilder(),
		env:     handler.NewEnv(0, "redisquota-nosession", gp),
